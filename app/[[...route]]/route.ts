...
function getCachedIpInfo(ip: string): unknown | null {
...
function setCachedIpInfo(ip: string, data: unknown): void {
  ipInfoCache.set(ip, {
    data,
    timestamp: Date.now(),
  });
...
// Get IP Information (Admin only)
app.get("/api/admin/ip-info/:ip", async (c) => {
  const authResult = requireAdminAuth(c.req.header("authorization") || null);
  ...
  try {
    ...
    // Fetch IP information from ip-api.com (HTTP only)
    const response = await fetch(
      `http://demo.ip-api.com/json/${ip}?fields=66842623`,
    );

    if (!response.ok) {
      return c.json({ error: "Erro ao buscar informações do IP" }, 500);
    }
